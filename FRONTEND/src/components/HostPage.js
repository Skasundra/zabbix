...
const HostPage = () => {
        ...
        if (query) {
          ...
          if (filterType && filterValue) {
            if (filterType === 'triggers') {
              setActiveTriggers((triggers) => triggers.filter((t) => t.priority.toString() === filterValue));
            } else if (filterType === 'alerts') {
              setAlerts((alerts) => alerts.filter((a) => a.priority.toString() === filterValue));
            } else if (filterType === 'problems') {
              setProblems((problems) => problems.filter((p) => p.severity.toString() === filterValue));
            } else {
              ...
              setAlerts((alerts) =>
                alerts.filter(
                  (a) => a.description.toLowerCase().includes(query) || a.priority.toString().includes(query)
                )
              );
              setProblems((problems) =>
                problems.filter(
                  (p) => p.name.toLowerCase().includes(query) || p.severity.toString().includes(query)
                )
              );
            }
          }
  ...
  useEffect(() => {
    const pollData = async () => {
      ...
      const cpu = results[2].status === 'fulfilled' && results[0].value.length > 0
        ? parseFloat(results[0].value[0].value)
        : null;
      const memory = results[1].status === 'fulfilled' && results[1].value.length > 0
        ? parseFloat(results[1].value[0].value)
        : null;
      const disk = results[2].status === 'fulfilled' && results[2].value.length > 0
  ...
  const handleFetchCpuLoad = async () => {
    ...
    if (data.length > 0) {
      ...
      setHistory((prev) => {
        const lastEntry = prev.length > 0
          ? { ...prev[prev.length - 1], cpu: parseFloat(data[0].value), timestamp }
          : { timestamp, cpu: parseFloat(data[0].value), memory: null, disk: null };
  ...
  const handleFetchMemoryUtilization = async () => {
    ...
    if (data.length > 0) {
      ...
      setHistory((prev) => {
        const lastEntry = prev.length > 0
          ? { ...prev[prev.length - 1], memory: parseFloat(data[0].value), timestamp }
          : { timestamp, cpu: null, memory: parseFloat(data[0].value), disk: null };
        return [...prev.slice(0, -1), lastEntry].slice(-60);
      });
    }
    setRefreshing(false);
  ...
  const handleFetchDiskUtilization = async () => {
    ...
    if (data.length > 0) {
      ...
      setHistory((prev) => {
        const lastEntry = prev.length > 0
          ? { ...prev[prev.length - 1], disk: parseFloat(data[0].value), timestamp }
          : { timestamp, cpu: null, memory: null, disk: parseFloat(data[0].value) };
        return [...prev.slice(0, -1), lastEntry].slice(-60);
      });
    }
    setRefreshing(false);
        ...
        <div>
          <HeaderCard sx={{
            ...
            <CardContent sx={{
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flex: { xs: '1 1 100%', sm: '0 1 auto' } }}>
                <Avatar sx={{
              ...
              <Box sx={{
                ...
                <ActionButton onClick={handleFetchCpuLoad}>Fetch CPU Load</ActionButton>
                <ActionButton onClick={handleFetchMemoryUtilization}>Fetch Memory</ActionButton>
                <ActionButton onClick={handleFetchDiskUtilization}>Fetch Disk</ActionButton>
          ...
          {error && (
            <Box sx={{
              ...
              <Typography variant="body1" fontSize={{ xs: '0.9rem', sm: '1rem' }}>{error}</Typography>
            </Box>
          )}

          {loading ? (
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh' }}>
              <CircularProgress size={60} sx={{ color: theme.palette.primary.contrastText }} />
            </Box>
          ) : (
            <>
              <StatsGridContainer>
                ...
                <Card sx={{
                  ...
                  <CardContent sx={{ height: '100%' }}>
                    {filteredHistory.length > 0 ? (
                      <Line data={cpuChartData} options={{ ...chartOptions, plugins: { ...chartOptions.plugins, title: { ...chartOptions.plugins.title, text: 'CPU Load Over Time' } } }} />
                    ) : (
                      <Typography
                        align="center"
                        color="textSecondary"
              ...
              <ChartContainer>
                <Card sx={{
                  ...
                  <CardContent sx={{ height: '100%' }}>
                    {filteredHistory.length > 0 ? (
                      <Line data={memoryChartData} options={{ ...chartOptions, plugins: { ...chartOptions.plugins, title: { ...chartOptions.plugins.title, text: 'Memory Utilization Over Time' } } }} />
                    ) : (
                      <Typography
                        align="center"
                        color="textSecondary"
              ...
              <ChartContainer>
                <Card sx={{
                  ...
                  <CardContent sx={{ height: '100%' }}>
                    {filteredHistory.length > 0 ? (
                      <Line data={diskChartData} options={{ ...chartOptions, plugins: { ...chartOptions.plugins, title: { ...chartOptions.plugins.title, text: 'Disk Utilization Over Time' } } }} />
                    ) : (
                      <Typography
                        align="center"
                        color="textSecondary"
              ...
              <Box ref={alertsRef}>
                <DataTableContainer
                  ...
                  columns={[
                    ...
                    { field: 'lastchange', label: 'Last Change', align: 'center' }
                  ]}
                  page={alertsPage}
                  rowsPerPage={rowsPerPage}
                ...
                >
                  {alerts.slice(alertsPage * rowsPerPage, alertsPage * rowsPerPage + rowsPerPage).map((alert, index) => (
              ...
              <Box ref={triggersRef}>
                <DataTableContainer
                  ...
                  columns={[
                    { field: 'description', label: 'Description' },
                    { field: 'priority', label: 'Priority', align: 'center' },
                    { field: 'expression', label: 'Expression', align: 'center' },
                    { field: 'lastchange', label: 'Last Change', align: 'center' },
                    ...
                    <StyledTableRow key={trigger.triggerid}>
                      ...
                      <TableCell align="center" data-label="Priority">
                        <StyledChip
                          ...
                          severity={priorityColors[trigger.priority] || 'info'}
                          size="small"
                        />
                      </TableCell>
              ...
              <Box ref={problemsRef}>
                <DataTableContainer
                  ...
                  columns={[
                    ...
                    { field: 'acknowledges', label: 'Acknowledged', align: 'center' }
                  ]}
                  page={problemsPage}
                  rowsPerPage={rowsPerPage}
                    ...
                    <StyledTableRow key={problem.eventid}>
                      ...
                      <TableCell align="center" data-label="Severity">
                        <StyledChip
                          ...
                          severity={priorityColors[problem.severity] || 'info'}
                          size="small"
                        />
                      </TableCell>
